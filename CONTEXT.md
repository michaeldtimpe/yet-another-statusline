...
for older payloads; **tok** = the **Effective Session Tokens** — every token
...
A single billing-weighted token figure for the *current* session, rendered as the `tok <count>` segment. Each token class is scaled by its price relative to the input rate — **Cache Read** 0.1×, cache write 1.25×, **Output** `rate_out/rate_in` (5× on every shipped model) — and the result is expressed in **input-token-equivalents**. This is exactly **Session Cost** divided by the input rate, so it tracks billed dollars while staying in token units. Computed by `TokenAccounting.effective_tokens`.
...
_Avoid_: presenting it as a literal count of tokens the model processed — it is a price-weighted equivalent, not a wire-level token count. Drives no colour. Drops before the protected `git`/`ctx`/`model` segments but survives longer than `cache`, `<rate>/m`, the rate-limit quotas, and the timestamp.
