...
class for this session weighted by its billing rate (cache read 0.1×, cache
write 1.25×, output 5×) and expressed in input-token-equivalents, so repeated
...
floor do the lowest-value segments drop (rate → cch → timestamp → plan/quota);
...
A single billing-weighted token figure for the *current* session, rendered as the `tkn <count>` segment. Each token class is scaled by its price relative to the input rate — **Cache Read** 0.1×, cache write 1.25×, **Output** `rate_out/rate_in` (5× on every shipped model) — and the result is expressed in **input-token-equivalents**. This is exactly **Session Cost** divided by the input rate, so it tracks billed dollars while staying in token units. Computed by `TokenAccounting.effective_tokens`.
_Why weighted, not raw_: a raw sum is dominated by **Cache Read**, which is re-counted on every turn (the same cached context, re-read), so a brand-new session would read as 1M+ even though almost nothing fresh was billed. Weighting collapses those re-reads to their true 0.1× cost.
